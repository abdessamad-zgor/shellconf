 ...
 * return is had to 
 * */
export function withCssColors(prompt) {
  let pColors = prompt.components.map(c => c.colors);
  let AnsiColorsToCSS = Object.keys(ANSIBasicColor).reduce((acc, cur) => {
  ...
  let reversedAnsiColors = Object.keys(AnsiColorsToCSS).reduce(
    (acc, cur) =>
      acc.set(ANSIBasicColor[cur], AnsiColorsToCSS[cur]), new Map()
  );
  for (let pColor of pColors) {
    pColor.background = reversedAnsiColors.get(pColor.background);
    pColor.foreground = reversedAnsiColors.get(pColor.foreground);
  }
  prompt.components = prompt.components.map((c, i) => ({ ...c, colors: pColors[i] }))
  return prompt;
}
