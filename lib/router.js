...
shellconfRouter.get("/prompt", withEnv((shellEnv) => (req, res) => {
  ...
  res.render("prompt", { prompts: shellPrompts, selectedPrompt: shellEnv.selectedPrompt, selectPrompt, isActive, title: "Prompt" })
}));
...
shellconfRouter.post("/prompt/select", withEnv((shellEnv) => (req, res) => {
  ...
  res.render('prompt-view', { prompt: toView(shellEnv.selectedPrompt) })
}));
